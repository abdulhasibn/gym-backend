...
export class CompleteDietItemUseCase {
  constructor(
  ...
  async execute(actor: AuthenticatedActor, gymOrgId: GymOrgId, itemId: string): Promise<void> {
    ...
    await this.logPrescribed.log({
      clientUserId: actor.userId,
      logDate: today,
      dietPlanMealItemId: found.item.id,
      foodItemId: found.item.foodItemId,
      servingId: found.item.servingId,
      quantity: found.item.quantity.value,
      mealSlot: found.meal.mealSlot,
    });
  }
}
